...
class CircuitPopup {
     ...
     */
    constructor() {
        this.allSeaonRacesInfo = null; //needs to be set in the Main class
        document
    ...
    handleCircuitPopup = (event) => {
        const currentRaceCircuit = this.allSeaonRacesInfo.find(
    ...
    displayDriverInfo(driverDetails) {
        ...
        driverPopupInfoSection.appendChild(
            Utility.createElement(
                "p",
                null,
                "Age: " + this.getAge(driverDetails.dob),
            ),
        );
...
class RaceInfoController {
    ...
    constructor(circuitPopup) {
        ...
        this.allSeaonRacesInfo = null; // set in the Main controller
    }
    /**
    ...
    populateWithRaceInfo = (raceID) => {
        const currentRaceInfo = this.allSeaonRacesInfo.find(
 ...
 * Initializes and manages all other controllers for popups, race details, race results and qualifying resutls.
 */
class Main {
    /**
    ...
    /**
     * Initializes the controllers with race and qualifying results and race info for the specific year in event.target.value.
     *
     * @param {Event} event
    ...
    async initialize(event) {
        try {
            [
                ...
                this.seasonRacesInfo,
            ] = await Promise.all([
                Utility.fetchWithCache(
                    `${API.BASE_URL}${API.QUERY.ALL_RACE_RESULTS}${event.target.value}`,
                ),
                Utility.fetchWithCache(
                    `${API.BASE_URL}${API.QUERY.ALL_QUALIFYING_RESULTS}${event.target.value}`,
                ),
                Utility.fetchWithCache(
                    `${API.BASE_URL}${API.QUERY.ALL_RACES_INFO}${event.target.value}`,
                ),
            ]);
            this.circuitPopup.allSeaonRacesInfo = this.seasonRacesInfo;
